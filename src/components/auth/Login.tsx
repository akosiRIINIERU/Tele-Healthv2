...
export const Login: React.FC = () => {
  ...
  const handleGoogleLogin = async () => {
    // Demo Google login
    toast.info('Google Sign-In is a demo feature. For production, complete setup at: https://supabase.com/docs/guides/auth/social-login/auth-google');
    setLoading(true);
    ...
    try {
      ...
      // If demo accounts don't exist, create a demo user
      toast.error('Demo account not found. Please use regular sign in or create an account.');
    } finally {
      setLoading(false);
    }
  };
          ...
          <Tabs value={role} onValueChange={(v: string) => setRole(v as 'patient' | 'doctor')}>
            ...
            <TabsContent value={role} className="mt-6">
              ...
              <Button
                ...
                disabled={loading}
                aria-label={`Sign in with Google as ${role}`}
              >
